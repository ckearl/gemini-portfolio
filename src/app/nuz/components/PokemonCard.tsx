...
export default function PokemonCard({
  ...
  if (pokemon) {
    return (
      <div className="w-full h-full flex flex-col justify-between relative">
        ...
        <div className="text-xs font-bold text-white bg-gray-800 px-2 py-1 rounded w-full text-center">
          {(pokemon.nickname || pokemon.name).toUpperCase()}
        </div>
        
        {/* Type Badges - in the middle row */}
        <div className="flex flex-col gap-1 justify-center">
          {pokemon.types.map((type) => (
            <span
              key={type}
              className={`px-2 py-1 rounded text-xs font-bold text-white border-2 border-white text-center ${typeColors[type] || 'bg-gray-500'}`}
            >
              {type.toUpperCase()}
            </span>
          ))}
  ...
  return (
    <button
      ...
      className="w-full h-full bg-gray-700 hover:bg-gray-600 border-2 border-dashed border-gray-500 hover:border-gray-400 rounded-lg flex items-center justify-center transition-colors"
    >
